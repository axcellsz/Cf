...
document.getElementById('setTokenBtn').addEventListener('click', () => {
  const t = prompt('Enter UI token (the token that authorizes UI -> worker):', getUiToken() || '');
  if (t !== null) setUiToken(t.trim());
});
document.getElementById('createBtn').addEventListener('click', async () => {
  const u = document.getElementById('username').value.trim();
  const d = document.getElementById('days').value.trim();
  if (!u || !d) { toast('username and days required'); return; }
  try {
    const res = await fetch('/api/create-user', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + getUiToken() },
      body: JSON.stringify({ username: u, days: Number(d) })
    });
    const j = await res.json();
    if (res.ok && j.success) {
      toast('Created'); refreshList();
    } else {
      toast('Create failed');
      console.error(j);
    }
  } catch(e){
    console.error(e); toast('Create error');
  }
});
